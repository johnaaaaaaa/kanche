...
'''
def send_mail(content,week_flag):
    week_flag=week_flag+"-周统计"
    week_flag=base64.encodestring(week_flag).split('\n')[0]
    smtp = smtplib.SMTP()
    ...
    smtp.sendmail('junwei.zhuang@kanche.com', 'dennis.zhao@kanche.com',
                  ...
                  'Subject: =?UTF-8?B?'+week_flag+'?=\r\n\r\n' +
                  quopri.encodestring(content))
    smtp.quit()

def statistics_week(week, ul,week_flag):
...
if __name__ == "__main__":
    ...
    week_start=(start-datetime.timedelta(days=7)).strftime('%y年%m月%d日')
    week_end=(start-datetime.timedelta(days=1)).strftime('%y年%m月%d日')
    week_flag=week_start+"-"+week_end
    week=(start-datetime.timedelta(days=7)-datetime.timedelta(hours=8),start-datetime.timedelta(hours=8))
    content = "本周\t新增注册用户数\t新增代运营用户数\t累计注册用户数\t累计注册代运营用户数\t有发车操作的用户数\t有发车操作的用户中的代运营用户数\t总发车数量\t本周累计车源总量\t本周在售车源总量\t代运营用户发车数量\t新增用户发车数量" + "\n"
    content += statistics_week(week, ul,week_flag)
    send_mail(content,week_flag)
