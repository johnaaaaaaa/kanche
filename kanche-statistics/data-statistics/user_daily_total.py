...
def statistics_daily(week, ul):
    ...
    content = str(cnt) + "\t" + str(scnt) + "\t" + str(zcnt_all) + "\t" + str(zcnt_all_D)+ "\t" + str(acnt2)+ "\t" + str(acnt)+ "\t" + str(xcnt)+ "\t" +str(sscnt)+"\t"+ str(acnt2_number)+ "\t" +str(wcnt)+"\t"+str(rcnt)+"\t"+ str(ant_number)+ "\t" + str(xcnt_number)+ "\t" + str(scnt_number) +"\n"
...
if __name__ == "__main__":
    ...
    content = "新增注册用户数\t新增代运营用户数\t累计注册用户数\t累计注册代运营用户数\t有发车操作的用户数\t有发车操作的用户中的新增用户数\t有发车操作的用户中的代运营用户数\t新增代运营用户有发车的用户\t总发车数量\t当日累计车源总量\t当日在售车源总量\t新增用户发车数量\t代运营用户发车数量\t新增代运营用户发车数量" + "\n"
    for daily in daily_list:
        #print daily,'\t'

        content+=statistics_daily(daily, ul)
    send_mail(content)
