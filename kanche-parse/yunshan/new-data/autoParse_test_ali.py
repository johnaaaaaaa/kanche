...
def job(data,data1,data4):
    ...
    base_vehicle_source_local=db.base_vehicle_source_ali
...
if __name__ == "__main__":
    ...
    iData = db.acarSrcContent_aliyun.find({}).sort([('_id',pymongo.ASCENDING)]).limit(maxRows)
    iObj = getOne(iData)
        ...
        for i in range(0,maxJob):
            ...
            if p:
                if p.exitcode != None:
                    ...
                    iData = db.acarSrcContent_aliyun.find({'_id':{'$gt':id}}).sort([('_id',pymongo.ASCENDING)]).limit(maxRows)
                    iObj = getOne(iData)
                    break
            else:
                id = iObj['_id']
